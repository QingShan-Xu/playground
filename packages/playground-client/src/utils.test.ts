...
import { addPackageJSONIfNeeded, bundleList, createError, createPackageJSON, customStore, generateHtml, generateUrl, getPkgFromUnpkg, getTemplate, normalizePath, nullthrows } from './utils'
...
describe('nullthrows', () => {
  it('should throw an error if the value is null or undefined', () => {
    expect(() => nullthrows(null)).toThrow('[playground-client]: Value is nullish')
    expect(() => nullthrows(undefined)).toThrow('[playground-client]: Value is nullish')
  })
...
describe('createPackageJSON', () => {
  it('should create a valid package.json string', () => {
    ...
    const result = createPackageJSON(name, entry, dependencies, devDependencies)
...
describe('normalizePath', () => {
  ...
  it('should normalize an object with paths as keys', () => {
    ...
    const result = normalizePath(paths)
    expect(result).toEqual({ '/path/one': 'content1', '/path/two': 'content2' })
  })
})
...
describe('generateUrl', () => {
  ...
  it('should generate URL for CSS files', () => {
    const item = { path: 'file.css', text: 'body { color: red; }' } as OutputFile
    generateUrl(item)
    expect(bundleList).toHaveLength(1)
...
describe('generateHtml', () => {
  it('should generate HTML with JS and CSS modules', () => {
    ...
    expect(result).toContain('<link rel="stylesheet" type="text/css" href="file.css">')
  })
})
